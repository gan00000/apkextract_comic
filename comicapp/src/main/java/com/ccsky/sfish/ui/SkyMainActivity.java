...
import com.ccsky.sfish.ui.scene.FavoritesScene;
import com.ccsky.sfish.ui.scene.GalleryCommentsScene;
import com.ccsky.sfish.ui.scene.GalleryDetailScene;
import com.ccsky.sfish.ui.scene.GalleryInfoScene;
import com.ccsky.sfish.ui.scene.GalleryListScene;
import com.ccsky.sfish.ui.scene.GalleryPreviewsScene;
import com.ccsky.sfish.ui.scene.HistoryScene;
import com.ccsky.sfish.ui.scene.ProgressScene;
import com.ccsky.sfish.ui.scene.QuickSearchScene;
...
public final class SkyMainActivity extends StageActivity
    ...
    protected Announcer getLaunchAnnouncer() {
        ...
        } else if (SkyUtils.needSignedIn(this)) {
    ...
    protected void onUnrecognizedIntent(@Nullable Intent intent) {
        ...
        if (clazz != null && SkySolidScene.class.isAssignableFrom(clazz)) {
    ...
    protected void onCreate2(@Nullable Bundle savedInstanceState) {
        ...
        mChangeTheme.setOnClickListener(v -> {
            ...
            ((SkyApplication) getApplication()).recreate();
        });
    ...
    private void checkCellularNetwork() {
        if (Network.getActiveNetworkType(this) == ConnectivityManager.TYPE_MOBILE) {
            showTip(R.string.cellular_network_warning, SkyBaseScene.LENGTH_SHORT);
        }
    }
    ...
    private boolean isSolid() {
        ...
        return topClass == null || SkySolidScene.class.isAssignableFrom(topClass);
    }
    ...
    public void onSceneViewCreated(SceneFragment scene, Bundle savedInstanceState) {
        ...
        if (scene instanceof SkyBaseScene && mRightDrawer != null && mDrawerLayout != null) {
            SkyBaseScene baseScene = (SkyBaseScene) scene;
    ...
    public void onSceneViewDestroyed(SceneFragment scene) {
        ...
        if (scene instanceof SkyBaseScene) {
            SkyBaseScene baseScene = (SkyBaseScene) scene;
    ...
    public void showTip(CharSequence message, int length) {
        if (null != mDrawerLayout) {
            Snackbar.make(mDrawerLayout, message,
                    length == SkyBaseScene.LENGTH_LONG ? 5000 : 3000).show();
        } else {
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_homepage) {
            ...
            Intent intent = new Intent(this, SkySettingsActivity.class);
