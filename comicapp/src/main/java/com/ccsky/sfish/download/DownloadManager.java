...
import com.ccsky.sfish.dao.DownloadLabel;
...
public class DownloadManager implements SpiderQueen.OnSpiderListener {
    ...
    public DownloadManager(Context context) {
        ...
        // Get all labels
        List<DownloadLabel> labels = SkyDB.getAllDownloadLabelList();
        mLabelList = labels;
        ...
        // Get all info
        List<DownloadInfo> allInfoList = SkyDB.getAllDownloadInfo();
        ...
        for (int i = 0, n = allInfoList.size(); i < n; i++) {
            ...
            if (list == null) {
                ...
                if (!containLabel(info.label)) {
                    ...
                    labels.add(SkyDB.addDownloadLabel(info.label));
                }
            }
            list.add(info);
        }
    ...
    public void addDownload(List<DownloadInfo> downloadInfoList) {
        for (DownloadInfo info: downloadInfoList) {
            ...
            if (null == list) {
                ...
                if (!containLabel(info.label)) {
                    // Add label to DB and list
                    mLabelList.add(SkyDB.addDownloadLabel(info.label));
                }
            }
            list.add(info);
            // Sort
    ...
    public void addDownloadLabel(List<DownloadLabel> downloadLabelList) {
        for (DownloadLabel label: downloadLabelList) {
            ...
            if (!containLabel(labelString)) {
                ...
                mLabelList.add(SkyDB.addDownloadLabel(label));
            }
        }
    }
    ...
    public void deleteDownload(long gid) {
        ...
        if (info != null) {
            // Remove from DB
            SkyDB.removeDownloadInfo(info.gid);
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                for (DownloadInfo downloadInfo : list) {
                    ...
                    UniFile file = downloadDir.findFile(".sfish");
                    if (file == null) {
                        continue;
                    }
    ...
    public void moveLabel(int fromPosition, int toPosition) {
        ...
        SkyDB.moveDownloadLabel(fromPosition, toPosition);
